...
/**
 * Retrieves sorted guesses based on clue direction and ID, then pads the array.
 *
 ...
 * @returns An array of guess strings, sorted and padded to MAX_SOLUTION_WORDS.
 */
export const getPaddedSortedGuesses = (
  ...
  // 6. Create the padded array matching the circuit input size
  const paddedGuesses = Array(MAX_SOLUTION_WORDS).fill(0n);
  for (let i = 0; i < sortedGuesses.length && i < MAX_SOLUTION_WORDS; i++) {
    paddedGuesses[i] = BigInt(stringToHex(sortedGuesses[i]));
  }
...
export const calculateSortedGuessesHash = (
  ...
  // 1. Get the sorted and padded guess strings
  const paddedSortedGuesses = getPaddedSortedGuesses(gameData, grid);

  // 2. Encode each guess string to a bigint Field element
  const encodedFields = paddedSortedGuesses.map(guess => BigInt(stringToHex(guess)));

  // 3. Calculate the Poseidon2 hash of the padded bigint array
  // Note: The array is already padded correctly by getPaddedSortedGuesses
  const finalHash = poseidon2Hash(encodedFields);
